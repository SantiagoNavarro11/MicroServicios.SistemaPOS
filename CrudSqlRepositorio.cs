namespace Utilitarios
{
    ...
    public class Funciones
    {
        ...
        // Llave secreta utilizada para el cifrado y descifrado de datos.
        private static readonly string Key = "^z863fkir5B4xX_4Q@-0?~jk";

        /// <summary>
        /// Encripta una cadena de texto usando el algoritmo AES en modo ECB y padding PKCS7.
        /// </summary>
        /// <param name="cadenaDatos">Cadena de texto plano que se desea encriptar.</param>
        /// <returns>Cadena encriptada en formato Base64.</returns>
        public static string Encriptar(string cadenaDatos)
        {
            ...
            byte[] bytesEntrada = Encoding.UTF8.GetBytes(cadenaDatos);
            byte[] bytesEncriptados = encryptor.TransformFinalBlock(bytesEntrada, 0, bytesEntrada.Length);

            return Convert.ToBase64String(bytesEncriptados);
        }

        /// <summary>
        /// Desencripta una cadena previamente cifrada usando AES en modo ECB con padding PKCS7.
        /// </summary>
        /// <param name="cadenaDatos">Cadena cifrada en formato Base64.</param>
        /// <returns>Cadena de texto plano original.</returns>
        public static string Desencriptar(string cadenaDatos)
        {
            using Aes aes = Aes.Create();
            aes.Key = Encoding.UTF8.GetBytes(Key);
            aes.Mode = CipherMode.ECB;
            ...
            byte[] bytesEncriptados = Convert.FromBase64String(cadenaDatos);
